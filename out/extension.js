...
const copilotTokenManager_1 = require("./copilotDirect/copilotTokenManager");
const copilotClient_1 = require("./copilotDirect/copilotClient");
const githubAuth_1 = require("./copilotDirect/githubAuth");
const modelsDirectClient_1 = require("./modelsDirect/modelsDirectClient");
...
function inferVscodeLmRightText(model) {
...
function inferDiscountLabelFromText(text) {
...
function inferVscodeLmAutoRightText(model) {
    const anyModel = model;
    const detail = typeof anyModel?.detail === 'string' ? String(anyModel.detail) : '';
    const tooltip = typeof anyModel?.tooltip === 'string' ? String(anyModel.tooltip) : '';
    return inferDiscountLabelFromText(`${detail}\n${tooltip}`) ?? inferVscodeLmRightText(model);
}
function isHostVisibleVscodeLmModel(model) {
    const m = model;
    ...
    try {
        if (m?.isHidden === true || m?.hidden === true)
            return false;
        if (m?.isUserVisible === false || m?.userVisible === false)
            return false;
        if (m?.isVisible === false || m?.visible === false)
            return false;
        if (m?.isUserSelectable === false || m?.userSelectable === false)
            return false;
        if (m?.isSelectable === false || m?.selectable === false)
            return false;
...
function makeSettingsModelsCacheKey(provider) {
    ...
    if (provider === 'models-direct') {
        const baseUrl = String(cfg.get('modelsDirect.baseUrl', '') ?? '').trim();
        const tagDefault = String(cfg.get('modelsDirect.modelTagDefault', '') ?? '').trim();
        return `models-direct:${baseUrl}|tagDefault=${tagDefault}`;
    }
    if (provider === 'copilot-direct') {
        const authMode = String(cfg.get('copilotDirect.authMode', 'minimal') ?? 'minimal');
...
const ASK_STUDY_ALLOWED_COMMAND_TOOL_NAMES = new Set([
// e.g. 'explainSelection'
]);
const ASK_STUDY_ALLOWED_MCP_SERVER_IDS = new Set([
// e.g. 'docs'
]);
const ASK_STUDY_ALLOWED_VSCODE_COMMAND_IDS = new Set([
...
function activate(context) {
    ...
    void (async () => {
        try {
            const cfg = vscode.workspace.getConfiguration('tripilot');
            const chatProvider = String(cfg.get('chatProvider', 'vscode-lm'));
            if (chatProvider !== 'copilot-direct')
                return;
            const authMode = String(cfg.get('copilotDirect.authMode', 'minimal'));
            const scopes = (0, githubAuth_1.getGitHubScopes)(authMode === 'permissive' ? 'permissive' : 'minimal');
            ...
            try {
                const peek = await (0, githubAuth_1.peekCopilotDirectDeviceFlowSession)(context.secrets, authMode);
                devicePeek = !!peek;
            }
            catch {
                // ignore
            }
            if (!hasSession && !devicePeek)
                return;
            ...
            try {
                const tokenManager = new copilotTokenManager_1.CopilotTokenManager(extensionVersion, () => (authMode === 'permissive' ? 'permissive' : 'minimal'), context.secrets);
                const client = new copilotClient_1.CopilotDirectClient(extensionVersion, editorVersionHeader);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('tripilot.debug.diagnoseCopilotDirect', async () => {
        ...
        const authMode = String(config.get('copilotDirect.authMode', 'minimal'));
        const scopes = (0, githubAuth_1.getGitHubScopes)(authMode === 'permissive' ? 'permissive' : 'minimal');
        ...
        const truncate = (s, max = 2000) => (s.length > max ? `${s.slice(0, max)}\n…(truncated ${s.length - max} chars)` : s);
        // 1) GitHub session
        ...
        try {
            const sess = await (0, githubAuth_1.getGitHubSession)(authMode === 'permissive' ? 'permissive' : 'minimal', {
        ...
        const probeModels = async (label, baseUrl) => {
            ...
            try {
                ...
                const arr = (Array.isArray(json?.data) ? json.data : undefined) ??
                ...
                if (Array.isArray(arr) && arr.length) {
                    ...
                    for (const mm of arr.slice(0, 8)) {
                        ...
                        const mult = mm?.billing?.multiplier ??
                            mm?.multiplier ??
    ...
    context.subscriptions.push(vscode.commands.registerCommand('tripilot.copilotDirect.signOut', async () => {
        const confirm = await vscode.window.showWarningMessage('将清除 Tripilot 的 copilot-direct device-flow 登录信息（仅影响 Tripilot，不会退出 VS Code 全局 GitHub 账号）。\n下次使用 copilot-direct 将需要重新登录。', { modal: true }, 'Sign out', '取消');
        if (confirm !== 'Sign out')
            return;
        const deleted = await (0, githubAuth_1.clearCopilotDirectDeviceFlowSessions)(context.secrets);
        try {
        ...
        void vscode.window.showInformationMessage(deleted > 0
...
function getAgentProfileMerged(id) {
    ...
    return (getAgentProfilesMerged().find((p) => p.id === normalized) ??
...
async function removeAgentProfile(profileId) {
    ...
    if (DEFAULT_AGENT_PROFILES.some((p) => p.id === id))
        return;
    const current = readAgentProfilesFromConfig();
...
class TripilotSettingsPanel {
    ...
    constructor(panel, context, mcpManager, extensionVersion, initialPage) {
        ...
        this.copilotTokenManager = new copilotTokenManager_1.CopilotTokenManager(this.extensionVersion, () => this.getCopilotAuthMode(), this.context.secrets);
        this.copilotClient = new copilotClient_1.CopilotDirectClient(this.extensionVersion, `vscode/${vscode.version}`);
        this.modelsDirectClient = new modelsDirectClient_1.ModelsDirectClient(this.extensionVersion, `vscode/${vscode.version}`);
        ...
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                ...
                case 'copilotDirectRelogin': {
                    ...
                    const confirm = await vscode.window.showWarningMessage(`将清除 Tripilot 的 copilot-direct 登录缓存并重新登录（${mode}）。`, { modal: true }, '继续', '取消');
                    if (confirm !== '继续')
                        return;
                    await (0, githubAuth_1.clearCopilotDirectDeviceFlowSessions)(this.context.secrets, mode);
                    this.resetCopilotDirectCaches();
                    clearSettingsModelsCache();
                    try {
                        const sess = await (0, githubAuth_1.getGitHubSession)(mode, {
                            clearSessionPreference: true,
                            secretStorage: this.context.secrets
                        });
                        void vscode.window.showInformationMessage(`signed in${sess.accountLabel ? `: ${sess.accountLabel}` : ''}${sess.source ? ` (${sess.source})` : ''}`);
                    }
                    catch (e) {
                ...
                case 'copilotDirectSignOut': {
                    const confirm = await vscode.window.showWarningMessage('将清除 Tripilot 的 copilot-direct device-flow 登录信息（仅影响 Tripilot，不会退出 VS Code 全局 GitHub 账号）。', { modal: true }, 'Sign out', '取消');
                    if (confirm !== 'Sign out')
                        return;
                    await (0, githubAuth_1.clearCopilotDirectDeviceFlowSessions)(this.context.secrets);
                    this.resetCopilotDirectCaches();
                    clearSettingsModelsCache();
                    await this.refreshAndPost(false);
                    return;
    ...
    static resetCopilotDirectCachesIfOpen() {
        try {
    ...
    getChatProvider() {
        const raw = String(vscode.workspace.getConfiguration('tripilot').get('chatProvider', 'vscode-lm') ?? 'vscode-lm');
        if (raw === 'copilot-direct')
            return 'copilot-direct';
        if (raw === 'models-direct')
            return 'models-direct';
        return 'vscode-lm';
    }
    getModelsDirectConfig() {
        const cfg = vscode.workspace.getConfiguration('tripilot');
        const baseUrl = String(cfg.get('modelsDirect.baseUrl', '') ?? '').trim();
        const apiKey = String(cfg.get('modelsDirect.apiKey', '') ?? '').trim();
        const defaultModel = String(cfg.get('modelsDirect.defaultModel', '') ?? '').trim();
        const modelTagDefault = String(cfg.get('modelsDirect.modelTagDefault', '') ?? '').trim();
        const extraFieldName = String(cfg.get('modelsDirect.extraFieldName', 'tripilot') ?? 'tripilot').trim();
        const modelTagsRaw = cfg.get('modelsDirect.modelTags', undefined);
        const modelExtrasRaw = cfg.get('modelsDirect.modelExtras', undefined);
        const modelTags = modelTagsRaw && typeof modelTagsRaw === 'object' ? modelTagsRaw : undefined;
        const modelExtras = modelExtrasRaw && typeof modelExtrasRaw === 'object' ? modelExtrasRaw : undefined;
        return {
            baseUrl,
    ...
    getCopilotAuthMode() {
        const raw = String(vscode.workspace.getConfiguration('tripilot').get('copilotDirect.authMode', 'minimal') ?? 'minimal');
    ...
    async computeModelsStatus(models) {
        ...
        if (provider === 'vscode-lm') {
            ...
            const hostVisibleSource = vscdbVisible?.ids?.size
    ...
    async getCopilotDirectAuthStatus() {
        const started = Date.now();
        const authMode = this.getCopilotAuthMode();
        const scopes = (0, githubAuth_1.getGitHubScopes)(authMode);
        const cfg = vscode.workspace.getConfiguration('tripilot');
        const tokenUrlOverride = String(cfg.get('copilotDirect.tokenUrl', '') ?? '').trim();
        const tokenUrlOverrideEnabled = !!tokenUrlOverride;
        const deviceFlowEnabled = !!cfg.get('copilotDirect.deviceFlow.enabled', true);
        const deviceFlowClientId = String(cfg.get('copilotDirect.deviceFlow.clientId', '') ?? '').trim();
        ...
        let vscodeAuthHasSession;
        let vscodeAuthAccountLabel;
        let vscodeAuthError;
        if (vscodeAuthAvailable) {
            try {
                ...
                if (isSettingsPerfEnabled()) {
                    settingsPerfLog(`[settings] authStatus vscode.authentication.getSession ${Date.now() - t}ms hasSession=${!!existing} ${this.perfPrefix()}`);
                }
            }
            catch (e) {
        ...
        let deviceFlowAccountLabel;
        let deviceFlowUpdatedAtMs;
        try {
            const t = Date.now();
            const peek = await (0, githubAuth_1.peekCopilotDirectDeviceFlowSession)(this.context.secrets, authMode);
            if (peek) {
            ...
            if (isSettingsPerfEnabled()) {
                settingsPerfLog(`[settings] authStatus peekDeviceFlow ${Date.now() - t}ms cached=${deviceFlowHasCachedToken} ${this.perfPrefix()}`);
            }
        }
        catch {
            // ignore
        }
        const preferredSource = vscodeAuthHasSession
            ? 'vscode-auth'
            : deviceFlowHasCachedToken
                ? 'device-flow'
                : undefined;
        const preferredAccountLabel = preferredSource === 'vscode-auth'
    ...
    async setCommandToolEnabled(name, enabled) {
        const toolName = String(name);
        const profile = getAgentProfileMerged(this.activeAgentProfileId);
        if (profile.id === 'ask-study' && enabled && !ASK_STUDY_ALLOWED_COMMAND_TOOL_NAMES.has(toolName))
            return;
        const set = new Set(profile.enabledCommandTools ?? []);
        if (enabled)
            set.add(toolName);
    ...
    async setMcpServerEnabled(id, enabled) {
        const serverId = String(id);
        const profile = getAgentProfileMerged(this.activeAgentProfileId);
        if (profile.id === 'ask-study' && enabled && !ASK_STUDY_ALLOWED_MCP_SERVER_IDS.has(serverId))
            return;
        const set = new Set(profile.enabledMcpServers ?? []);
        if (enabled)
            set.add(serverId);
    ...
    async getAllModelsForSettings() {
        ...
        if (provider === 'copilot-direct') {
            try {
                ...
                if (isSettingsPerfEnabled()) {
                    settingsPerfLog(`[settings] getAllModelsForSettings copilot.listModels ${Date.now() - tList}ms count=${models.length} ${this.perfPrefix()}`);
                }
                const mapped = models.map((m) => ({
                    id: m.id,
                    name: m.displayName ?? m.id,
        ...
        if (provider === 'models-direct') {
            ...
            try {
                ...
                if (isSettingsPerfEnabled()) {
                    settingsPerfLog(`[settings] getAllModelsForSettings models-direct listModels ${Date.now() - tList}ms count=${models.length} ${this.perfPrefix()}`);
                }
                const mapped = models.map((m) => ({
                    id: m.id,
                    name: m.displayName ?? m.id,
        ...
        if (isSettingsPerfEnabled()) {
            settingsPerfLog(`[settings] getAllModelsForSettings vscode.lm.selectChatModels ${Date.now() - tSelect}ms count=${chats.length} ${this.perfPrefix()}`);
        }
        const real = chats
            .map((m) => ({
            id: m.id,
...
class TripilotChatViewProvider {
    ...
    lastModels = [];
    lastCopilotModels = [];
    lastCopilotAutoDiscount;
    copilotAutoPrefetchInFlight;
    lastModelsDirectModels = [];
    selectedModelId;
    ...
    constructor(context, mcpManager, extensionVersion, editorVersionHeader) {
        ...
        this.copilotClient = new copilotClient_1.CopilotDirectClient(this.extensionVersion, this.editorVersionHeader);
        this.modelsDirectClient = new modelsDirectClient_1.ModelsDirectClient(this.extensionVersion, this.editorVersionHeader);
        this.copilotTokenManager = new copilotTokenManager_1.CopilotTokenManager(this.extensionVersion, () => this.getCopilotAuthMode(), this.context.secrets);
    ...
    buildCopilotDirectLmModels() {
        ...
        const allRealModels = this.lastCopilotModels
            .map((m) => {
        ...
        if (!visibleModelIds.size || visibleModelIds.has('auto')) {
            const now = Date.now();
            const autoDiscount = this.lastCopilotAutoDiscount && this.lastCopilotAutoDiscount.expiresAtMs > now
    ...
    prefetchCopilotAutoDiscount(stateHint) {
        // Non-blocking: fetch /models/session and then re-post model list to show Auto discount.
        const now = Date.now();
        if (this.lastCopilotAutoDiscount && this.lastCopilotAutoDiscount.expiresAtMs - 30_000 > now)
            return;
        if (this.copilotAutoPrefetchInFlight)
            return;
    ...
    async ensureCopilotAutoSession(state) {
        const now = Date.now();
    ...
    mapCustomAgentToolsToOptionalBuiltinTools(toolSpecs) {
        ...
        for (const raw of toolSpecs) {
            ...
            if (t === 'search') {
                for (const n of TOOL_SETS.search.tools)
                    enabled.add(n);
                continue;
            }
            if (t === 'edit') {
                for (const n of TOOL_SETS.edit.tools)
                    enabled.add(n);
                continue;
            }
            if (t === 'terminal') {
    ...
    getEffectiveAgentProfile(profileId) {
        ...
        if (!agent)
            return getAgentProfileMerged(profileId);
    ...
    ensureCopilotSystemInstructionUpToDate(state) {
    ...
    async ensureAskStudySandboxInitializedIfPossible() {
        if (this.selectedAgentProfileId !== 'ask-study')
            return;
    ...
    computeEnabledMcpServerIds(profile, isAskStudy, mcpConfigs, serverReferences) {
        const mcpEnabledSetRaw = profile.enabledMcpServers
            ? new Set(profile.enabledMcpServers.map((s) => String(s).trim()).filter(Boolean))
            : new Set(mcpConfigs.filter((s) => s.enabled).map((s) => s.id));
        const mcpEnabledSet = isAskStudy
            ? new Set(Array.from(mcpEnabledSetRaw).filter((id) => ASK_STUDY_ALLOWED_MCP_SERVER_IDS.has(id)))
            : mcpEnabledSetRaw;
    ...
    async buildToolsForRequest(profileId, serverReferences, toolReferences) {
        ...
        const effectiveServerRefs = isAskStudy
            ? new Set(Array.from(serverReferences).filter((s) => ASK_STUDY_ALLOWED_MCP_SERVER_IDS.has(s)))
            : serverReferences;
        const effectiveToolRefs = isAskStudy ? new Set(Array.from(toolReferences)) : toolReferences;
        ...
        const enabledServerIds = this.computeEnabledMcpServerIds(profile, isAskStudy, mcpConfigs, effectiveServerRefs);
        ...
        try {
            if (this.getChatProvider() === 'copilot-direct') {
                ...
                if (!this.lastCopilotModels.length) {
                    const text = 'Copilot /models 返回空列表，因此模型菜单只会显示 Auto。' +
            ...
            if (this.getChatProvider() === 'models-direct') {
                ...
                const allRealModels = this.lastModelsDirectModels
                    .map((m) => {
            ...
            const lmAny = vscode.lm;
            if (!lmAny || typeof lmAny.selectChatModels !== 'function') {
                const text = '当前 VS Code 环境不支持 Language Model API（vscode.lm）。请使用支持该 API 的 VS Code 版本，或在对应发行版中启用/安装语言模型提供方。';
                if (state) {
                    this.postToHost(state, { type: 'chatAppend', role: 'tool', text });
                }
                else {
            ...
            const allRealModels = hostVisibleModels
                .map((m) => ({
                id: m.id,
                name: m.name,
                vendor: m.vendor,
                ...
                version: m.version,
                maxInputTokens: m.maxInputTokens,
                rightText: inferVscodeLmRightText(m) ??
    ...
    getSelectedModel() {
        ...
        if (this.selectedModelId && this.selectedModelId !== 'auto') {
            ...
            if (exact && isHostVisibleVscodeLmModel(exact))
                return exact;
        }
    ...
    async runCopilotDirectRequest(state, args) {
        const built = await this.buildToolsForRequest(this.selectedAgentProfileId, args.directives.serverReferences, args.directives.toolReferences);
        state.copilotConversation ??= [];
        ...
        const finalText = await runCopilotDirectToolCallingLoop({
            ...
            policy: {
                agentProfileId: this.selectedAgentProfileId,
                askStudySandboxDir: String(vscode.workspace.getConfiguration('tripilot').get('askStudySandboxDir', '.tripilot/ask-study') ??
                    '.tripilot/ask-study')
            },
            maxIterations: vscode.workspace.getConfiguration('tripilot').get('maxToolIterations', 6),
    ...
    async runModelsDirectRequest(state, args) {
        const cfg = this.getModelsDirectConfig();
        if (!cfg.baseUrl) {
            throw new Error('未配置 tripilot.modelsDirect.baseUrl');
        }
    ...
    async handleUserMessage(text, state) {
        ...
        try {
            if (chatProvider === 'copilot-direct') {
                ...
                if (!modelId) {
                    throw new Error('未检测到 Copilot 可用模型。\n' +
            ...
            const built = await this.buildToolsForRequest(this.selectedAgentProfileId, directives.serverReferences, directives.toolReferences);
            ...
            try {
                wantsWrite =
                    /\b(save|write|edit|apply|append|prepend|update)\b|写入|保存|修改|编辑|创建.*文件|应用并保存|追加|添加|插入|写到|写进|写入到|保存到|加到文件|写进文件|落盘/i.test(String(text ?? ''));
                hasWriteTools =
                ...
                if (wantsWrite && !hasWriteTools) {
                    ...
                    const hint = '当前对话未启用“写入/编辑”类工具，因此无法直接应用并保存到工作区文件。\n' +
            ...
            if (wantsWrite && hasWriteTools && !didInvokeEditTool) {
                try {
                    ...
                    const diag = `(诊断) profile=${this.selectedAgentProfileId} model=${this.selectedModelId || 'auto'} ` +
                ...
                if (diffBlock) {
                    try {
                        await executeToolCall('apply_patch', { input: diffBlock, explanation: 'Recovered from assistant-provided diff block.' }, {
                            abortSignal: state.abortController.signal,
                            requestEditsApproval: (a) => this.requestEditsApproval(state, a),
                            postEditsReview: (a) => this.postEditsReview(state, a),
                            mcpManager: this.mcpManager,
...
async function runLmToolCallingLoop(args) {
    const truncateForWebview = (text, max = 20_000) => {
        const s = String(text ?? '');
        if (s.length <= max)
            return s;
        return s.slice(0, max) + `\n…(truncated, ${s.length - max} chars)`;
    };
    const runSubagent = async ({ prompt, description }) => {
        ...
        const subConversation = [
            vscode.LanguageModelChatMessage.User(`[Subagent]\nTask: ${description}\n` +
    ...
    const lmTools = args.toolDefinitions
    ...
    for (let iteration = 0; iteration < args.maxIterations; iteration++) {
        ...
        const response = await args.model.sendRequest(args.conversation, {
            tools: lmTools,
...
async function runModelsDirectToolCallingLoop(args) {
    ...
    const runSubagent = async ({ prompt, description }) => {
        ...
        const subConversation = [
            {
                role: 'system',
                content: `You are a context-isolated subagent. Task: ${description}.\n` +
                    'Work autonomously and return ONLY the final result. Do not include intermediate reasoning or tool logs.'
            },
    ...
    const tools = args.toolDefinitions.filter((t) => args.enabledTools.has(t.function.name));
    for (let iteration = 0; iteration < args.maxIterations; iteration++) {
        args.abortSignal.throwIfAborted?.();
        if (args.token.isCancellationRequested)
            throw new Error('Canceled.');
...
function toOriginalEol(textLf, eol) {
...
function levenshteinWithin(a, b, maxDistance) {
    ...
    if (bl === 0)
        return al;
    if (bl > al)
        return levenshteinWithin(s2, s1, maxDistance);
    let prev = new Array(bl + 1);
    let curr = new Array(bl + 1);
    for (let j = 0; j <= bl; j++)
        prev[j] = j;
...
function buildWhitespaceFlexibleRegex(pattern) {
...
function suggestClosestSnippet(fileTextLf, targetLf) {
    ...
    if (bestStart < 0 || bestCount !== 1)
        return undefined;
    if (bestScore < Math.max(2, window))
        return undefined;
...
async function executeToolCall(name, input, runtime) {
    const policy = runtime?.policy;
    const isAskStudy = policy?.agentProfileId === 'ask-study';
    const tripilotCfg = vscode.workspace.getConfiguration('tripilot');
    const editsApprovalMode = String(tripilotCfg.get('edits.approvalMode', 'post') ?? 'post');
    ...
    const sensitiveGlobsRaw = tripilotCfg.get('edits.sensitiveGlobs', defaultSensitiveGlobs);
    ...
    const isSensitiveRelPath = (relPosix) => {
    ...
    const ensureWorkspaceAndMaybeSandboxForAbsolutePath = (uri) => {
        if (uri.scheme !== 'file')
    ...
    const assertRelPathInSandbox = (relativePath) => {
        if (!isAskStudy)
            return;
    ...
    const resolveCwdFsPathForAskStudy = (cwdRel) => {
        const { folder, rel: sandboxRel, uri: sandboxUri } = getAskStudySandbox();
        if (!cwdRel)
            return sandboxUri.fsPath;
    ...
    if (runtime?.commandTools?.has(name)) {
        ...
        const result = await vscode.commands.executeCommand(cfg.command, ...args);
    ...
    if (runtime?.mcpManager) {
        ...
        if (dot > 0) {
            ...
            const lmToolName = (0, mcpClient_1.makeMcpLmToolName)(serverId, toolName);
    ...
    switch (name) {
        ...
        case 'read_file': {
            const { filePath, startLine, endLine, offset, limit } = parsed;
            ...
            const asFiniteNumber = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : undefined);
            ...
            if (hasV2 && !hasV1) {
                ...
                if (rawOffset !== undefined && rawOffset > lineCount) {
                    throw new Error(`Invalid offset ${rawOffset}: file only has ${lineCount} line${lineCount === 1 ? '' : 's'}. Line numbers are 1-indexed.`);
                }
        ...
        case 'grep_search': {
            const { query, isRegexp, includePattern, maxResults, includeIgnoredFiles } = parsed;
        ...
        case 'create_file': {
            await ensureSandboxDir();
            const { filePath, content, overwrite } = parsed;
            const uri = resolveAnyPathToUri(filePath);
            // Enforce workspace-only for absolute paths; ask&study also enforces sandbox.
        ...
        case 'apply_patch': {
            ...
            const patchRaw = (() => {
                ...
                if (parsed && typeof parsed === 'object' && parsed._parseError && typeof parsed.raw === 'string') {
            ...
            const previews = [];
            const actionByUri = new Map();
            const folder = vscode.workspace.workspaceFolders?.[0];
            if (!folder)
                throw new Error('No workspace folder opened.');
            const workspaceRoot = folder.uri.fsPath;
            const ensureWorkspaceFileAndGetRelPosix = (uri) => {
                if (uri.scheme !== 'file') {
            ...
            const stripGitPrefix = (name) => {
                let n = String(name ?? '').trim();
                // parsePatch often yields "a/path" and "b/path".
                n = n.replace(/^(a|b)\//, '');
                return n;
            };
            const isDevNull = (name) => {
            ...
            const obj = parsed && typeof parsed === 'object' ? parsed : {};
            const edits = Array.isArray(obj?.edits) ? obj.edits : undefined;
            if (edits && edits.length) {
                const globalEnableHealing = !!vscode.workspace.getConfiguration('tripilot').get('edits.enableHealing', false);
                ...
                for (const [filePath, fileEdits] of grouped) {
                    ...
                    for (const e of fileEdits) {
                        ...
                        if (!oldString) {
                            throw new Error(`Missing required field oldString for ${relPath}.\n\n` +
                                'Next steps:\n' +
                        ...
                        if (res.matchType === 'noChange') {
                            throw new Error(`This edit makes no changes in ${relPath} (oldString and newString are identical).\n\n` +
                                'Next steps:\n' +
                                '1) Remove this edit entry, or change newString.\n');
                        }
                        if (res.matchType === 'multiple') {
                            throw new Error(`Couldn't apply the edit in ${relPath} because the target text is ambiguous (matched ${res.occurrences} times).\n\n` +
                                'Next steps:\n' +
                        ...
                        if (res.matchType === 'expectedMismatch') {
                            throw new Error(`Couldn't apply the edit in ${relPath}: expected ${expectedReplacements} replacements, but found ${res.occurrences}.\n\n` +
                                'Next steps:\n' +
                        ...
                        if (res.matchType === 'none') {
                            ...
                            const retryTemplate = res.suggestion
                                ...
                                : '';
                            throw new Error(`Couldn't find the text to replace in ${relPath}.\n\n` +
                        ...
                        if (res.updatedText === updated) {
                            throw new Error(`The edit didn't change ${relPath}.\n\n` +
                                'Next steps:\n' +
                    ...
                    const kind = original ? 'update' : 'add';
                ...
                const filePatches = (0, diff_1.parsePatch)(patchRaw);
                const tryAppendOnly = (originalText, fp) => {
                ...
                const trySingleFileAppendFromRaw = async () => {
                    ...
                    let removedCount = 0;
                    for (const ln of lines) {
                        if (ln.startsWith('+++') || ln.startsWith('---') || ln.startsWith('diff --git') || ln.startsWith('index ') || ln.startsWith('@@'))
                            continue;
                        if (ln.startsWith('-')) {
                            removedCount++;
                            continue;
                ...
                if (!Array.isArray(filePatches) || filePatches.length === 0) {
                    ...
                    for (const fp of filePatches) {
                        ...
                        if (kind === 'delete') {
                            ...
                            const applied = (0, diff_1.applyPatch)(original, fp);
                            if (applied === false) {
                                const appended = tryAppendOnly(original, fp);
                                if (!appended) {
                                    throw new Error(`Couldn't apply the patch to ${relPath} because the file content doesn't match the diff context.\n\n` +
                                        'Next steps:\n' +
            ...
            const needsPreApproval = isAskStudy ||
        ...
        case 'get_changed_files': {
            const { repositoryPath, sourceControlState } = parsed;
            const folder = vscode.workspace.workspaceFolders?.[0];
        ...
        case 'run_in_terminal': {
            await ensureSandboxDir();
            const { command, explanation, isBackground } = parsed;
            ...
            const child = process.platform === 'win32'
                ? (0, node_child_process_1.spawn)('powershell', ['-NoProfile', '-Command', cmd], { cwd, windowsHide: true })
                : (0, node_child_process_1.spawn)('bash', ['-lc', cmd], { cwd });
            const rec = {
                id,
                command: cmd,
        ...
        case 'create_and_run_task': {
            await ensureSandboxDir();
            const { workspaceFolder, task } = parsed;
            ...
            if (!wsFolder || !commandExe)
                throw new Error('workspaceFolder and task.command are required.');
            ...
            const cwd = isProbablyAbsolutePath(wsFolder)
                ...
                : folder
                    ...
                    : wsFolder;
            const id = createUuid();
            const child = (0, node_child_process_1.spawn)(commandExe, args, { cwd, windowsHide: true });
            const rec = {
                id,
                label,
        ...
        case 'run_task': {
            await ensureSandboxDir();
            const { workspaceFolder, id } = parsed;
            const wsFolder = String(workspaceFolder ?? '').trim();
            const taskId = String(id ?? '').trim();
            if (!wsFolder || !taskId)
                throw new Error('workspaceFolder and id are required.');
        ...
        case 'get_task_output': {
            ...
            if (!rec)
                throw new Error('Unknown task id (only captured tasks are supported).');
            return JSON.stringify({
                id: rec.id,
                label: rec.label,
                done: rec.done,
        ...
        case 'run_tests': {
            ...
            const args = ['test'];
            const id = createUuid();
            const child = (0, node_child_process_1.spawn)(cmd, args, { cwd: folder.uri.fsPath, windowsHide: true });
            const out = { stdout: '', stderr: '' };
            child.stdout?.on('data', (b) => (out.stdout += String(b ?? '')));
            child.stderr?.on('data', (b) => (out.stderr += String(b ?? '')));
            const exitCode = await new Promise((resolve) => child.on('close', (c) => resolve(typeof c === 'number' ? c : null)));
            ...
            return JSON.stringify({ id, exitCode, stdout: truncate(out.stdout, 60000), stderr: truncate(out.stderr, 60000) }, null, 2);
        }
        case 'runTests': {
        ...
        case 'copilot_getNotebookSummary': {
            ...
            const items = cells.map((c, i) => {
                ...
                const md = c?.metadata;
                const tripilotCellId = md && typeof md === 'object' ? String(md.tripilotCellId ?? '') : '';
                ...
                return {
                    ...
                    execution: exec
                        ? {
                            success: Boolean(exec.success),
                            startTime: exec.timing?.startTime ?? null,
                            endTime: exec.timing?.endTime ?? null
                        }
                        : null,
        ...
        case 'run_notebook_cell': {
            ...
            while (Date.now() < deadline) {
                ...
                const exec = latest.executionSummary;
                if (now.text !== before.text || (exec && exec.timing?.endTime)) {
                    break;
                }
            }
            ...
            const exec = final.executionSummary;
            return JSON.stringify({
                filePath: uri.fsPath,
                cellId: `cell-${idx}`,
                index: idx,
        ...
        case 'edit_notebook_file': {
            await ensureSandboxDir();
            const { filePath, cellId, editType, newCode, language } = parsed;
            if (isAskStudy && isProbablyAbsolutePath(filePath)) {
            ...
            const nb = originalText ? JSON.parse(originalText) : { nbformat: 4, nbformat_minor: 5, metadata: {}, cells: [] };
            ...
            if (editType === 'insert') {
                ...
                if (isTop || isBottom)
                    throw new Error('delete requires a concrete cellId (not TOP/BOTTOM).');
                if (typeof resolvedIndexFromToolId !== 'number' || !Number.isFinite(resolvedIndexFromToolId)) {
                ...
                if (isTop || isBottom)
                    throw new Error('edit requires a concrete cellId (not TOP/BOTTOM).');
                if (typeof resolvedIndexFromToolId !== 'number' || !Number.isFinite(resolvedIndexFromToolId)) {
                ...
                if (!cell.metadata || typeof cell.metadata !== 'object')
                    cell.metadata = {};
                if (!cell.metadata.tripilotCellId)
                    cell.metadata.tripilotCellId = createUuid();
            }
            ...
            let resolved = { cellId: null, stableId: null, tripilotCellId: null, index: null };
            try {
                ...
                if (idx < 0)
                    idx = 0;
                if (idx >= afterCells.length)
                    idx = Math.max(0, afterCells.length - 1);
                const cell = afterCells[idx];
                const md = cell?.metadata;
                const tripilotIdRaw = md && typeof md === 'object' ? String(md.tripilotCellId ?? '') : '';
                resolved = {
                    cellId: `cell-${idx}`,
        ...
        case 'agent.runSubagent': {
            const { prompt, description } = parsed;
            if (!runtime?.runSubagent)
                throw new Error('runSubagent is not available in this runtime.');
            const result = await runtime.runSubagent({ prompt: String(prompt ?? ''), description: String(description ?? '') });
            return JSON.stringify({ result }, null, 2);
        }
        case 'fetch_webpage': {
            if (isAskStudy)
                throw new Error('ask&study 不允许联网抓取网页（fetch_webpage）。');
            const { urls, query } = parsed;
            ...
            for (const u of list.slice(0, 5)) {
                ...
                // Pre-approval
                const pre = await vscode.window.showWarningMessage(`Tripilot 将访问外部 URL：${u}`, { modal: true }, '允许一次', '取消');
                if (pre !== '允许一次') {
                ...
                const post = await vscode.window.showWarningMessage(`Tripilot 已抓取内容（预览）：\n${preview}\n\n是否将其提供给模型作为上下文？`, { modal: true }, '纳入上下文', '取消');
                if (post !== '纳入上下文') {
        ...
        case 'github_repo': {
            if (isAskStudy)
                throw new Error('ask&study 不允许访问 GitHub（github_repo）。');
            const { repo, query } = parsed;
            ...
            const pre = await vscode.window.showWarningMessage(`Tripilot 将访问 GitHub API：${apiUrl}`, { modal: true }, '允许一次', '取消');
            if (pre !== '允许一次') {
            ...
            const post = await vscode.window.showWarningMessage(`Tripilot 将把 GitHub 搜索结果（${matches.length} 条）提供给模型。是否继续？`, { modal: true }, '继续', '取消');
            if (post !== '继续') {
        ...
        case 'install_extension': {
            if (isAskStudy)
                throw new Error('ask&study 不允许安装扩展（install_extension）。');
            const { id, name } = parsed;
            const extId = String(id ?? '').trim();
            if (!extId)
                throw new Error('id is required.');
            const ok = await vscode.window.showWarningMessage(`Tripilot 请求安装扩展：${String(name ?? extId)} (${extId})`, { modal: true }, '安装', '取消');
            if (ok !== '安装') {
        ...
        case 'vscode_searchExtensions_internal': {
            if (isAskStudy)
                throw new Error('ask&study 不允许搜索/安装扩展（vscode_searchExtensions_internal）。');
            const { keywords, ids } = parsed;
        ...
        case 'run_vscode_command': {
            ...
            const result = await vscode.commands.executeCommand(id, ...a);
            return safeToString(result);
        }
        case 'run_command': {
            const { commandId, args } = parsed;
            ...
            if (!id)
                throw new Error('commandId is required.');
            return await executeToolCall('run_vscode_command', { commandId: id, name: `run ${id}`, args: Array.isArray(args) ? args.map(String) : [] }, runtime);
        }
        case 'create_new_workspace': {
            await ensureSandboxDir();
            if (isAskStudy)
                throw new Error('ask&study 不允许创建新 workspace（create_new_workspace）。');
            const { query } = parsed;
            const folder = vscode.workspace.workspaceFolders?.[0];
            if (!folder)
                throw new Error('No workspace folder opened.');
        ...
        case 'new_workspace': {
            await ensureSandboxDir();
            if (isAskStudy)
                throw new Error('ask&study 不允许创建新 workspace（new_workspace）。');
            const { name: rawName, directory: rawDir, template: rawTemplate } = parsed;
            const folder = vscode.workspace.workspaceFolders?.[0];
            if (!folder)
                throw new Error('No workspace folder opened.');
        ...
        case 'get_project_setup_info': {
            ...
            const info = { projectType: t, detected: [], install: [], build: [], test: [], run: [], notes: [] };
            if (t === 'python-script') {
        ...
        case 'get_vscode_api': {
            ...
            const candidates = await vscode.workspace.findFiles('**/{node_modules/vscode/vscode.d.ts,node_modules/@types/vscode/index.d.ts}', undefined, 5);
            if (!candidates.length) {
        ...
        case 'list_code_usages': {
            const { symbolName, filePaths } = parsed;
        ...
        case 'workspace_listFiles': {
            const { relativeDir, recursive } = parsed;
            const folder = vscode.workspace.workspaceFolders?.[0];
            if (!folder)
                throw new Error('No workspace folder opened.');
        ...
        case 'workspace_findInFiles': {
            const { pattern, isRegExp, isCaseSensitive, isWordMatch, include, exclude, maxResults } = parsed;
            const results = await findInFiles({
                pattern,
                isRegExp: Boolean(isRegExp),
        ...
        case 'workspace_applyEdits': {
            await ensureSandboxDir();
            const { edits, requireConfirmation, allowPreview } = parsed;
            ...
            for (const e of edits) {
                ...
                const range = new vscode.Range(new vscode.Position(Math.max(0, e.startLine - 1), Math.max(0, e.startCharacter - 1)), new vscode.Position(Math.max(0, e.endLine - 1), Math.max(0, e.endCharacter - 1)));
        ...
        case 'diagnostics_list': {
            const { relativePath, maxItems } = parsed;
        ...
        case 'process_run': {
            const { command, args, cwd, timeoutMs, maxOutputChars } = parsed;
            const folder = vscode.workspace.workspaceFolders?.[0];
